...
Requires the Apify connector with the shared MMA token. If a scrape returns an auth or connection error, the connector isn't configured yet. Tell the user to open the setup guide and follow it: [Apify Scraping Setup for Claude MMA](https://mmaglobalcom.sharepoint.com/:w:/s/MMAGlobal/IQCxuNjI070YQ629dqM9p9arAQ0VfeMXGCTEsspdWz19az0?e=pFW5Fz) (MMA Internal › General › AI MMA Upskilling › AI Tool Onboarding for Staff). One-time manual paste; Claude can't set the token.
